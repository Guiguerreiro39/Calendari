...
const MobileEvents: React.FC = () => {
  const [currentDayContainer] = useAtom(CalendarAtoms.currentDayContainer)

  return (
    <>
      {currentDayContainer && currentDayContainer?.events.length > 0 && (
        <div className='px-4 py-10 sm:px-6 lg:hidden'>
          <ol className='divide-y divide-neutral-100 overflow-hidden rounded-lg bg-white text-sm shadow ring-1 ring-black ring-opacity-5'>
            {currentDayContainer.events.map((event, index) => (
